		...
		\begin{table}[hbt!]\caption{Historia de Usuario 1}% title of Table
			...
			\resizebox{15cm}{!} {
				\begin{tabular}{|c|c|}% centered columns (3 columns)
					...
					\hline
					\begin{tabular}[c]{@{}l@{}}Descripción: Como un visitante de la\\ página web quiero generar una\\ letra de una canción a partir de una palabra\\ o palabras que proporcione.\end{tabular}
					& \begin{tabular}[c]{@{}l@{}}Validación: Con la palabra o palabras\\ proporcionadas por el usuario con ayuda del modelo\\ entrenado se generará una letra musical\\ relacionada a estas.\end{tabular} \\
					\hline
		...
		\begin{table}[hbt!]\caption{Historia de Usuario 2}% title of Table
			...
			\resizebox{15cm}{!} {
				\begin{tabular}{|c|c|}% centered columns (3 columns)
					...
					& \begin{tabular}[c]{@{}l@{}}Usuario: Visitante\end{tabular} \\% inserting body of the table 
					\hline
					\begin{tabular}[c]{@{}l@{}}Descripción: Como un visitante de la\\ página web quiero poder descargar\\ la letra de la canción que se generó\\ a partir de la palabra o palabras que\\ proporcioné.\end{tabular}
		...
		\begin{table}[hbt!]\caption{Historia de Usuario 4}% title of Table
			...
			\resizebox{15cm}{!} {
				\begin{tabular}{|c|c|}% centered columns (3 columns)
					...
					& \begin{tabular}[c]{@{}l@{}}Usuario: Visitante\end{tabular} \\% inserting body of the table 
					\hline
					\begin{tabular}[c]{@{}l@{}}Descripción: Me gustaría generar otra\\ letra de canción con los mismo parámetros y\\ palabra(s) que ya había introducido.\end{tabular}
		...
		\begin{table}[hbt!]\caption{Historia de Usuario 5}% title of Table
			...
			\resizebox{15cm}{!} {
				\begin{tabular}{|c|c|}% centered columns (3 columns)
					...
					\hline                  % inserts single horizontal line
					\begin{tabular}[c]{@{}l@{}}Nombre: Validación de la palabra o palabras introducidas.\end{tabular}
					& \begin{tabular}[c]{@{}l@{}}Usuario: Sistema\end{tabular} \\% inserting body of the table 
					\hline
					\begin{tabular}[c]{@{}l@{}}Descripción: La página web validará la palabra\\ o palabras proporcionadas por el usuario para\\ con ellas hacer la petición al servidor\\ y generar una letra de canción relacionada a estas. \end{tabular}
					& \begin{tabular}[c]{@{}l@{}}Validación: Con ayuda del modelo y la palabra\\ o palabras introducidas y procesadas por la página web\\ se genera y despliega la letra generada.\end{tabular} \\
					\hline
...
\section{Base de Datos} % Explicar como se sacó la Base de Datos
		...
		En este apartado se expondrá por qué escogimos la base de datos que escogimos, qué tablas se pueden derivar de ella y el peso total del dataset que se utilizará para el entrenamiento del modelo.
		
		\subsection{Género Musical} % Explicar por qué se sacó de este género musical
		El género musical que elegimos es Rock en inglés ya que el dataset que encontramos de este tipo de música fue el más extenso y menos usado por la competencia, otros géneros contemplados fueron Hip Hop y Pop que también cuentan con un extenso catálogo de datos pero ya hay modelos que usan esos géneros.
		
		\subsection{Dataset escogido}
		El dataset seleccionado tiene por nombre "output.csv" siendo el formato del tipo ".csv" el más aceptado por Python para trabajar con datos muy grandes precisamente para proyectos de una índole similar. Este dataset tienen un total de [insete número] canciones qe incluye a diversos artistas siendo gracias a esto que el peso del archivo se va a los [peso] Mb.\\
		
		El dataset escogido proviene de la página [Vungalou?] e incluye 6 tipos de géneros de música los cuales son:\\
		\begin{itemize}
			\item Rock
			\item Pop
			\item Hip Hop
		...
		Al necesitar extraer información precisa para el entrenamiento del modelo no es necesario que la base de datos sea muy compleja, razón por la cual las tablas mostradas en el diagrama entidad-relación de aquí abajo se ven así, siendo lo de mayor relevancia el mostrar que un artista puede tener muchas canciones pero una canción le pertenece únicamente a un artista.
...
\section{Limpieza de la Base de Datos} % Explicar cómo se limpió la base de datos
		\subsection{Descripción}
		En esta sección se hablará del proceso seguido para limpiar la base de datos que se obtuvo, se mostrará la forma de evaluar los "errores" y con este criterio poder limpiar la base de datos.
...
\section{Modelo para Neural Network} % Explicar cómo se limpió la base de datos
		...
		\subsubsection{Verso}
		\begin{itemize}
			\item Estará compuesto de 4 a 6 oraciones y el valor será decidido por un número random.
			\item Las palabras que serán predichas en todo el verso serán de 4 a 5 y el valor será decidido por un número random.
		\end{itemize}
		
		\subsubsection{Pre-Coro y Puente}
		\begin{itemize}
			\item Estará compuesto de 2 a 3 oraciones y el valor será decidido por un número random.
			\item Las palabras que serán predichas en todo el verso serán de 6 a 10 y el valor será decidido por un número random.
		\end{itemize}
	
		\subsubsection{Coro}
		\begin{itemize}
			\item Estará compuesto de 3 a 4 oraciones y el valor será decidido por un número random.
			\item Las palabras que serán predichas en todo el verso serán de 3 a 8 y el valor será decidido por un número random.
		\end{itemize}
	...
	\subsection{Herramientas para generar el modelo}
