...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
 \chapter{\textcolor{azulescom}{Marco teórico}}
	    ...
	    \section{Apache}
	 	Es un servidor web gratuito y de código abierto el cual permite que los desarrolladores de sitios web puedan desplegar el contenido de ellas en un sitio web, este no es un servidor que se encuentre físicamente, sino que se trata un software ql cual ejecuta en un servidor. Su función es instaurar la conexión entre un servidor y los visitantes del sitio web mientras estos intercambian archivos entre ellos (siguiendo una estructura cliente-servidor). El servidor y el cliente interactúan mediante el protocolo \acrshort{http}, y el software de Apache es el encargado de mantener una comunicación ágil y segura entre las 2 partes.\\\\
	 	Apache trabaja sin problemas con otros sistemas de gestión de contenido (Joomla, Drupal, etc.), marcos de trabajo (Django, Laravel, etc.), y lenguajes de programación. Con todo esto lo vuelve una opción sólida al momento de escoger entre los distintos tipos de plataformas de alojamiento web, como serían \acrfull{vps} o alojamiento compartido.\\
...
\newpage
    	...
    	\section{Herramientas a usar}
            ...
            \paragraph{MongoDB \\}
            MongoDB es un sistema de base de datos multiplataforma dirigido a documentos, de esquema libre, es decir, que cada entrada o registro puede contar con un esquema de datos distinto, con atributos o columnas que pueden variar o no de un registro a otro.\\\\
            Sus características más destacadas son la  velocidad y su sencillo sistema de consulta de la base de datos. Alcanzando así un balance perfecto entre rendimiento y funcionalidad. 
            ...
            El modelo más común utilizado en este sistema de gestión de base datos, es el modelo relacional, donde se almacenan y proporcionan acceso a puntos de datos relacionados entre sí. Esto es posible a que cada fila de la tabla cuenta con un registro con un ID único, las columnas contienen atributos de los datos, y cada registro en su mayoría cuenta con un valor para cada atributo, lo que facilita el establecimiento de las relaciones entre los datos.\\\\
...
\section{Modelo para Neural Network} % Explicar cómo se limpió la base de datos
		...
		Habiendo hecho el análisis del modelo anterior podemos mostrar una versión ligeramente modificada que se puede usar en el proyecto, de forma tal que la última palabra [RIMA] esté dada por el porcentaje de coincidencia seleccionado por el usuario y que tiene como objetivo rimar con la última palabra de la penúltima oración previa: 
		\begin{figure}[H]
	...
	En este apartado se explicará de manera general cómo es que se hará la generación del modelo en la nube desde que se alimenta la información de entrenamiento hasta que se obtiene el resultado.
	
	\section*{Herramientas para generar el modelo}
	...
	A continuación se muestra el proceso seguido para el entrenamiento de ML:
	...
	Dentro del apartado podemos asignar las siguientes actividades como parte de las iteraciones 1 y 2, las cuales se centran:
	\begin{itemize}
		\item Colección de datos
		\item Integración de los datos
		\item Preparación y limpieza de los datos
	\end{itemize}
	
	\begin{figure}[H]
		\includegraphics[width=15cm]{./imagenes/Disenio/Iteracion_4/ml_process_e1.jpg}
		\centering 
		\caption{Diagrama de diseño del modelo}
	\end{figure}
	
	Habiendo visto el proceso de alimentación de datos para el ML podemos ahora mostrar de manera general el proceso que tomaremos para el entrenamiento de nuestro modelo, proceso que puede resumirse en el siguiente diagrama:
	\begin{figure}[H]
	...
	\begin{figure}[H]
		\includegraphics[width=12cm]{./imagenes/Disenio/Iteracion_4/sagemaker_localmode.jpg}
		\centering 
		\caption{Diagrama }
	\end{figure}
	...
	\begin{figure}[H]
		\includegraphics[width=12cm]{./imagenes/Disenio/Iteracion_4/roadtrip_sagemaker.jpg}
		\centering 
		\caption{Diagrama del posible proceso a seguir para la generación del modelo}
	\end{figure}
