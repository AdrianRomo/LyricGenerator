...
%				\end{figure}
		...
		Una base de datos generalmente es manejada por un \Gls{Sistema} de gestión de base de datos (DBMS). En conjunto, los datos y el \ac{DBMS}, junto con las aplicaciones asociadas a ellos, se les conoce como un sistema de base de datos.\cite{refQueEsBD}\par
...
%				En esta se tratan de almacenar los objetos completos (\Gls{Estado} y comportamiento), incorpora todos los conceptos del paradigma orientado a objetos (encapsulamiento, herencia, polimorfismo)\par
		...
		\section{Flask}
		Flask es un marco (framework) web, el cual es un modulo de Python el cual permite desarrollar aplicaciones web. No cuenta con un Manejador de Objetos Realacionales u \ac{ORM} por sus siglas en inglés, pero si cuenta con características como el enrutamiento de URLS y un motor de plantillas. En general es un marco de aplicación web WSGI.\cite{refFlask3}\par
		\ac{WSGI} (Interfaz de Puerta de enlace del Servidor Web) la cual es una especificación que describe como se va a comunicar un servidor web con una aplicación web, y como se pueden llegar a enlazar distintas aplicaciones web para procesar una solicitud o una petición.\cite{refFlask2}\par
...
%	    Una de las técnicas de autentificación que actualmente se usa es ''recordar la sesión'' usando las ''cookies'', en la siguiente sección, nos adentraremos en definir qué son las cookies y exponer sus vulnerabilidades.
		...
		\\
		Otro cambio
		Azure y \acrlong{gcp} GCP puntúan ligeramente mejor que AWS, esto no necesariamente significa que una plataforma es mejor que otra.\\
		\\
...
\end{comment}
    ...
    \begin{thebibliography}{20}
		...
		https://determined.ai/blog/cloud-v-onprem/ (July, 2020) adsfadf
