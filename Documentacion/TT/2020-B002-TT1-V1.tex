...
%				\end{figure}
				...
				La neurona es un tipo de célula que representa la unidad estructural y funcional del sistema nervioso. Su función consiste en transmitir información a través de impulsos nerviosos o químicos, desde un lugar del cuerpo hacia otro.\\\\
...
%				\end{figure}
        ...
        El Transformer en \acrfull{pln} es una arquitectura que tiene como objetivo codificar cada palabra de una frase en función del resto de la secuencia, permitiendo así introducir el contexto en la representación matemática del texto.\\\\    
        ...
        Una base de datos es una colección organizada de información o datos, los cuales pertenecen a un mismo contexto, se encuentran almacenados de forma física o digital con la finalidad de realizar distintas acciones como consultas futuras, ingreso de nuevos datos, actualización o eliminación de estos.\\\\
...
%			\acrshort{html}5 es una mejora de esta combinación, lo que unió todos estos elementos. \acrshort{html}5 propone estándares para cada aspecto de la Web y también un propósito claro para cada una de las tecnologías involucradas. A partir de esto, \acrshort{html} provee los elementos estructurales, \acrshort{css} se concentra en como volver esta estructura utilizable y atractiva a la vista, y JavaScript tiene todo lo necesario para brindar dinamismo y construir aplicaciones web completamente funcionales. Cabe mencionar que \acrshort{html}5 funciona diferente dependiendo del navegador y la versión en la que se esté trabajando, algunos soportan más carcterísticas o diferentes funcionalidades que otros.
			...
			\acrfull{css} es el lenguaje para describir la presentación de las páginas web. Permite adaptar la presentación a diferentes tipos de dispositivos. \acrshort{css} es independiente de \acrshort{html} y se puede utilizar con cualquier lenguaje de marcado basado en \acrshort{xml}. La separación de \acrshort{html} de \acrshort{css} hace que sea más fácil mantener los sitios, compartir hojas de estilo entre páginas y adaptar las páginas a diferentes entornos. \cite{refcss}	
...
%			Oficialmente \acrshort{css} no tiene nada que ver con \acrshort{html}5, es un complemento desarrollado para superar las limitaciones y reducir la complejidad de \acrshort{html}. Al principio, atributos dentro de las etiquetas \acrshort{html} proveían estilos esenciales para cada elemento, pero a medida que \acrshort{html} evolucionó, la escritura de códigos se volvió más compleja y \acrshort{html} por sí mismo no pudo satisfacer más las demandas de los diseñadores.\\\\
...
%			La versión 3 de \acrshort{css} sigue el mismo camino, pero esta vez con un mayor compromiso. La especificación de \acrshort{html}5 fue desarrollada considerando \acrshort{css} a cargo del diseño, Debido a esta consideración, la integración entre \acrshort{html} y \acrshort{css} es ahora vital para el desarrollo web y esta razón por la que cada vez que mencionamos \acrshort{html}5 también estamos haciendo referencia a \acrshort{css}3, aunque oficialmente se trate de dos tecnologías completamente separadas. Las nuevas características incorporadas en \acrshort{css}3 están siendo implementadas e incluidas junto al resto de la especificación en navegadores compatibles con \acrshort{html}5 \cite{refElGranLibro}.
			...
			JavaScript es un lenguaje de programación o secuencias de comandos que permite implementar funciones complejas en las páginas web. JavaScript permite crear contenido dinámico, controlar multimedia, animar imágenes, etc. \cite{refjs}
...
%			Para aprovechar esta prometedora plataforma de trabajo ofrecida por los nuevos navegadores, JavaScript fue expandido en cuestión de portabilidad e integración, a la vez, interfaces de programación de aplicaciones (\acrshort{api}s) fueron incorporando por defecto con cada navegador para asistir a JavaScript en funciones elementales. El objetivo de esto, fue principalmente hacer disponible funciones a través de técnicas de programación sencillas y estándares, expandiendo el alcance del lenguaje y facilitando la creación de programas útiles para la Web \cite{refElGranLibro}.
		    ...
		    \paragraph{OpenSSL.\\}
           Consiste en un robusto paquete de herramientas de administración y bibliotecas relacionadas con la criptografía, que suministran funciones criptográficas a otros paquetes como OpenSSH y navegadores web (para el acceso seguro a sitios \acrshort{https}). Estas herramientas ayudan al sistema a implementar el \acrfull{ssl}, así como otros protocolos relacionados con la seguridad, como el \acrfull{tls}. OpenSSL también permite crear certificados digitales que pueden aplicarse a un servidor, por ejemplo Apache \cite{refopenssl}.
            ...
            El modelo relacional, para el modelado y la gestión de bases de datos, es donde se almacenan y se proporcionan acceso a puntos de datos relacionados entre sí. Esto gracias a que cada fila de la tabla es un registro con un ID único, las columnas de la tabla contienen atributos de los datos, y cada registro generalmente cuenta con un valor para cada atributo, lo que facilita el establecimiento de las relaciones entre los puntos de datos.\\\\
			...
			\paragraph{\acrshort{gpt}. \\}
			\acrfull{gpt} es un transformer aprovechado para realizar tanto aprendizaje supervisado como aprendizaje no supervisado para el \acrfull{pln}.\\\\        
			\acrshort{gpt} es el sucesor de \acrshort{gpt}, es un gran modelo de lenguaje basado en transformers con 1500 millones de parámetros, entrenado en un conjunto de datos de 8 millones de páginas web. \acrshort{gpt} se entrena con un objetivo simple: predecir la siguiente palabra, dadas todas las palabras anteriores dentro de un texto. \cite{refQueesgpt}\\\\
...
\section{Arquitectura del sistema.}
	        ...
	        -modelado de la red
	        -base de datos
	        \subsection{Diseño de la aplicación web}
	        A continuación, se muestra cómo se pretende que se vea la aplicación web una vez terminada y funcionando.	        
	        ...
	        A continuación, se presentan algunos de los requerimientos funcionales mas importantes de nuestro proyecto:
	        \begin{itemize}
	        	\item Permitirá que cualquier usuario que acceda a la página, genere su letra musical, sin ninguna restricción.
	        	\item Permitirá que cualquier usuario puede descargar la letra generada, a través de un archivo de texto.
	        	\item Permitirá que el usuario puede generar otra letra musical distinta, usando los mismos parámetros que previamente había utilizado.
	        	\item La generación de las letras musicales, solo se generarán si se cumplen con todos los valores pedidos al usuario, los cuales serán (una palabra y un porcentaje que defina la cantidad de rimas dentro de la letra)
	        \end{itemize}
        	Ahora, se presentan algunos de los requerimientos no funcionales más importantes de nuestro proyecto 
	        \begin{itemize}
		        \item La página debe ser capaz de procesar mas de 10 transacciones a la vez.
		        \item La funcionalidad de la generación de letras musicales debe de responder al usuario en menos de 10 segundos.
		        \item La pagina debe de ser capaz de operar adecuadamente con hasta 50 usuarios a la vez.
		        \item El tiempo de aprendizaje del sistema por un usuario deberá de ser menor a 5 minutos.
		        \item El pagina debe proporcionar mensajes de de error que sean informativos y que ayuden al usuario.
	        ...
	        Los derechos de autor son una forma de protección para obras originales de autoría fijadas en un medio de expresión tangible. Los derechos de autor cubren tanto las obras publicadas como las no publicadas o inéditas.\cite{refCopyright}\\\\
	        ...
	        Los derechos de autor protegen la forma en que se expresan las ideas. Esta expresión es la forma única en la que las palabras, notas musicales, colores, formas, etc. son elegidas y organizadas u ordenadas. Es la expresión lo que hace que una obra sea original. Esto significa que puede haber muchos trabajos diferentes sobre la misma idea y todos de ellos estarán protegidos por derechos de autor, siempre y cuando expresen esta idea de una manera original.\cite{refOriginalidadWipo}\\\\
	        Según la Ley Federal del Derecho de Autor, la forma en cómo se expresa un autor al momento de crear su obra es lo que es protegido por la ley ante acciones fraudulentas.\cite{refOriginalidadLFDA}\\\\
