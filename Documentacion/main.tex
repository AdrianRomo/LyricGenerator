...
\makeglossaries
\newglossaryentry{Palabra1}
{
	name=Palabra1,
	description={Es una Palabra 1.\cite{refRAE}}
}
...
\newglossaryentry{Maquinas de Estados Finitos}{
    ...
    description={Conocidas también con Finite State Machines por su traducción en inglés, nos sirven para realizar procesos bien definidos en un tiempo discreto. Reciben una entrada, realizan un proceso y nos entregan una salida.\cite{refMaquinasFinitas}}
}

%\acrlong{}  \acrshort{} 
\newacronym{acronimo1}{ACRONIMO1}{Significado}
...
%   %   %   %   %   %   %   %   %	%
   ...
   \textcolor{guindapoli}{\part{Trabajo Terminal I}}
	...
	%\section{Introducción.}
	    ...
	    \begin{table}[htbp]
	    	\begin{tabular}{|p{5cm}|p{5cm}|p{5cm}|}
	    		...
	    		\hline
	    		Boredhumans - lyrics\_generator & Aplicación web en el que la IA fue entrenada con una base de datos con miles de letras para generar letras de canciones totalmente nuevas. La letra que crea es única y no una copia de alguna que exista actualmente, sin embargo, no permite customizar la letra. & Gratuito\\
	    		\hline
	    		...
	    		\hline
	    		Nuestra propuesta & Aplicación web que haciendo uso de una IA va a generar letras musicales originales a partir de un género musical en exclusivo, lo que asegurará un resultado final con coros y versos distintos cada vez que se utilice. & Gratuito\\
	    		\hline
	    	\end{tabular}
	    	\caption{Resumen de productos similares comparados con nuestra propuesta}
	    \end{table}
	    ...
	    \section{Metodolog\'ia.}
    		...
    		A continuación se indica cada una de las etapas de la metodología con lo que se realizar\'a en cada una de estas:\cite{Pressman}.
    		\begin{enumerate}
    		    \item \lipsum[16]
    		        \begin{itemize}
    		            \item \lipsum[17]
	...
	\chapter{\textcolor{azulescom}{Marco teórico}}
		...
		Decidimos solicitar este paquete que tiene Github para estudiantes debido a los distintos servicios que este ofrece al ser estudiantes, como lo son el acceso a de hosteo en DigitalOcean, Servicios en la nube y recursos de aprendizaje en Microsoft Azure, un Dyno personalizable en Heroku, entre otros. De entre todos los servicios que ofrecen, el que más interesa es la posibilidad de hospedar aplicaciones web por un periodo de 1 año, como lo es namecheap.\par
		Para ello se requiere entrar a la siguiente página, https://education.github.com/pack donde se iniciará  sesión con una cuenta de Github, una vez iniciada la sesión se pedirá un correo electrónico el cual esté  vinculado con una institución educativa, en este caso se introdujo el correo institucional.\par
		Se envía un correo para verificar que el correo sea válido, una vez verificado el correo electrónico introducido, en la página web a un lado de este aparecerá el nombre de la institución educativa, en este caso, el Instituto Politécnico Nacional. Y por último se pide agregar sobre que vamos a hacer en la plataforma.\par
		\begin{figure}[H]
			\includegraphics[width=12cm]{./imagenes/MarcoTeorico/GitHub/GitS1.png}
			\centering 
			\caption{Github realacionado con el correo institucional}
		\end{figure}
		Una vez enviada la solicitud con la información aparecerá un mensaje en el cual dice que obtendremos una respuesta sobre la solicitud en menos de 22 días.\par
		\begin{figure}[H]
			\includegraphics[width=12cm]{./imagenes/MarcoTeorico/GitHub/GitS2.png}
			\centering 
			\caption{Tiempo de espera de aprovación de la solicitud}
		\end{figure}
		En este caso no se obtuvo una respuesta en forma de correo electrónico por parte de Github, sino que simplemente a la cuenta con la que solicitamos este paquete de estudiante se le otorgo, es posible verificar esto entrando a la cuenta checando el perfil, en la parte inferior aparecerá un icono de pro, con el cual se nos da el acceso solicitado.
		\begin{figure}[H]
		...
		\section{Markovify}
		\subsection{¿Qué es markovify?}
		Markovify es un generador de cadenas de Markov simple y extensible. En este momento, su uso principal es construir modelos de Markov de grandes corpus de texto y generar oraciones aleatorias a partir de estos.
		\subsection{Razones de usar markovify}
		\begin{itemize}
			\item Los modelos se pueden almacenar como JSON, lo que le permite almacenar en caché sus resultados y guardarlos para más adelante.
			\item Los métodos de análisis de texto y generación de oraciones son altamente extensibles, lo que le permite establecer sus propias reglas.
		\end{itemize}
		\subsection{¿Cómo se usa markovify?}
		Lo primero que se tiene que hacer es obtener el texto como una cadena de texto (String), para esto usamos lo siguiente, lo que nos permite obtener la cadena de texto del archivo que queremos usar como corpus para markovify.
		\begin{minted}{python}
			with open("/path/a/mi/corpus.txt") as f:
			text = f.read()  
		\end{minted}
		Una vez obtenido el corpus debemos generar el modelo, para eso se hace uso del siguiente código:
		\begin{minted}{python}
			text_model = markovify.Text(text) 
		\end{minted}
		Después de haber generado el modelo, es comenzar a crear oraciones con este modelo para ello se pueden hacer de la siguiente manera:
		\begin{minted}{python}
			print(text_model.make_sentence())
			print(text_model.make_short_sentence(250))
		\end{minted}
		Con estas instrucciones, se generan oraciones aleatorias con base al corpus con el que se creó el modelo, si usamos make\_short\_sentence(x), las oraciones generadas serán de no más de x número de caracteres.\par
		El tamaño del estado es la cantidad de palabras de las que depende la probabilidad de una palabra siguiente. De forma predeterminada, markovify.Text() usa un tamaño de estado de 2. Pero se puede crear una instancia de un modelo con un tamaño de estado diferente usando lo siguiente:
		\begin{minted}{python}
			text_model = markovify.Text(text, state_size=4)
		\end{minted}
		Haciendo uso de markovify se pueden combinar uno o más modelos usando markovify.combine() dicha función acepta 2 argumentos
		\begin{itemize}
			\item \textbf{Modelos (models): }Son una lista de objetos markovify para combinar. Pueden ser instancias de markovify.Chain o markovify.Text (o sus subclases), pero todas deben ser del mismo tipo.
			\item \textbf{Pesos (weights): }Es una lista (la longitud exacta de los modelos) de enteros o flotantes que indican cuánto énfasis relativo colocar en cada fuente. Predeterminado: [1, 1, ...]. Este argumento es opcional.
		\end{itemize}
		Por ejemplo:
		\begin{minted}{python}
		...
		\end{minted}
		En este caso se combina el modelo a con el modelo b y a demás se le de un 50\% mas de peso a las conexiones del modelo a.\par
		Cuando se tiene generado un modelo, también se puede compilar para mejorar la velocidad de generación de texto y reducir el tamaño del modelo. Para ello hacemos uso de la siguiente instrucción.
		\begin{minted}{python}
			text_model = text_model.compile()
		\end{minted}
		Actualmente, los modelos compilados no se pueden combinar con otros modelos usando markovify.combine(). Si se desea combinar modelos, primero se deben combinar y luego se compila el resultado de esa combinación.
		\textbf{Notas: }Si no se va a trabajar markovify con texto se hace uso de la clase markovify.Chain().\par
		Markovify funciona mejor con textos grandes y bien puntuados. Si el texto no usa puntos para delimitar oraciones, se debe colocar cada oración en una nueva línea y usar la clase markovify.NewlineText() en lugar de la clase markovify.Text().
		...
		* Running on http://127.0.0.1:5000/
		...
		\begin{table}[ht]\caption{Tabla Comparativa de distintos Servidores Web}% title of Table
			...
			\resizebox{15cm}{!} {
				\begin{tabular}{c c c}% centered columns (3 columns)
					...
					\hline
					\begin{tabular}[c]{@{}l@{}}De código abierto y gratuito,\\incluso para su uso comercial.\\ Software estable y confiable.\\Frecuentemente actualizado\\incluyendo actualizaciones\\ regulares a los parches de\\ seguridad.
					\end{tabular}
					& \begin{tabular}[c]{@{}l@{}}Nginx es uno de los servidores\\ web que soluciona el problema\\ c10k y probablemente el más\\ exitoso al hacerlo, no crea un\\ nuevo proceso por cada solicitud y\\ en su lugar, maneja toda solicitud\\ entrante en un único hilo.\end{tabular}
					...
					\hline
					\begin{tabular}[c]{@{}l@{}}Funciona de forma intuitiva, con\\ sitios web hechos con WordPress.\\ Comunidad grande y posibilidad\\ de contactar con soporte\\ disponible de manera sencilla en\\ caso de cualquier problema. 
					\end{tabular}
					& \begin{tabular}[c]{@{}l@{}}El modelo basado en eventos de\\ Nginx distribuye solicitudes de\\ usuario entre procesos\\ trabajadores de una manera\\ eficiente, llevando con ello a una\\ mejor escalabilidad.\end{tabular}
					& \begin{tabular}[c]{@{}l@{}}\end{tabular} \\
					\hline
					\begin{tabular}[c]{@{}l@{}}Entre sus desventajas\\ se pueden llegar a tener\\problemas de rendimiento en\\ sitios web con tráfico\\ extremadamente pesado.\\Tantas opciones de configuración\\ pueden llevar a vulnerabilidades\\ de seguridad.
					\end{tabular}
		...
		\begin{itemize}
		    \item \textbf{Palabras en negritas:}
        \end{itemize}
